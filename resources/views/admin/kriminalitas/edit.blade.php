...
@section('title', 'Edit Data Kriminalitas')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="fw-bold"><i class="fas fa-edit me-2 text-accent"></i>Edit Data Kriminalitas</h4>
...
<div class="card border-0 shadow-sm app-card">
    <div class="card-body">
        <form action="{{ route('admin.kriminalitas.update', $kriminalita->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="kecamatan_id" id="kecamatan_id" class="form-select @error('kecamatan_id') is-invalid @enderror" required>
                        ...
                        @foreach ($kecamatans as $kec)
                            <option value="{{ $kec->id }}" {{ old('kecamatan_id', $kriminalita->kecamatan_id) == $kec->id ? 'selected' : '' }}>
                                {{ $kec->nama_kecamatan }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3 position-relative">
                    ...
                    <input type="text" id="tahun_display" class="form-control @error('tahun') is-invalid @enderror"
                           value="{{ old('tahun', $kriminalita->tahun) }}" placeholder="Pilih tahun" readonly autocomplete="off">
                    <input type="hidden" name="tahun" id="tahun" value="{{ old('tahun', $kriminalita->tahun) }}">
                    @error('tahun')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="jenis_kriminalitas" id="jenis_kriminalitas" class="form-select @error('jenis_kriminalitas') is-invalid @enderror" required>
                        ...
                        @foreach ($jenisOptions as $opt)
                            <option value="{{ $opt }}" {{ old('jenis_kriminalitas', $kriminalita->jenis_kriminalitas) == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" name="jumlah_kasus" id="jumlah_kasus" min="0"
                           ...
                           value="{{ old('jumlah_kasus', $kriminalita->jumlah_kasus) }}" required>
                    @error('jumlah_kasus')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
    ...
    <select name="tingkat_kerawanan" id="tingkat_kerawanan" class="form-select @error('tingkat_kerawanan') is-invalid @enderror">
        ...
        <option value="rendah" {{ old('tingkat_kerawanan', $kriminalita->tingkat_kerawanan) == 'rendah' ? 'selected' : '' }}>Rendah</option>
        <option value="sedang" {{ old('tingkat_kerawanan', $kriminalita->tingkat_kerawanan) == 'sedang' ? 'selected' : '' }}>Sedang</option>
        <option value="tinggi" {{ old('tingkat_kerawanan', $kriminalita->tingkat_kerawanan) == 'tinggi' ? 'selected' : '' }}>Tinggi</option>
    </select>
    ...
    @enderror
            ...
            <div class="mb-3">
                ...
                <input type="text" name="alamat_detail" id="alamat_detail" maxlength="255"
                       ...
                       value="{{ old('alamat_detail', $kriminalita->alamat_detail) }}">
                @error('alamat_detail')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="kelurahan" id="kelurahan" maxlength="255"
                           ...
                           value="{{ old('kelurahan', $kriminalita->kelurahan) }}">
                    @error('kelurahan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="latitude" id="latitude"
                           ...
                           value="{{ old('latitude', $kriminalita->latitude) }}" placeholder="Contoh: -3.4300">
                    @error('latitude')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="longitude" id="longitude"
                           ...
                           value="{{ old('longitude', $kriminalita->longitude) }}" placeholder="Contoh: 104.2300">
                    @error('longitude')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
...
<style>
    /* ===== Tema selaras dengan halaman peta (Peta Kriminalitas Kota Prabumulih) ===== */
    :root {
        --accent: #ff4d6d;
    ...
    /* ===== Year Picker (tema terang, selaras dengan kartu putih) ===== */
...
<script>
    ...
    // Saat halaman edit dibuka, langsung tampilkan batas kecamatan & marker sesuai data yang tersimpan
    @php
        $selectedKecamatanId = old('kecamatan_id', $kriminalita->kecamatan_id);
        $selectedLat = old('latitude', $kriminalita->latitude);
        $selectedLng = old('longitude', $kriminalita->longitude);
    @endphp

    @if ($selectedKecamatanId)
        showBoundary({{ $selectedKecamatanId }});
    @endif
